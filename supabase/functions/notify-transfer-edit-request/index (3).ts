...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f8fafc; color: #1e293b; padding: 24px 16px; margin: 0;">
  <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; border: 1px solid #e2e8f0; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
    ...
    <div style="padding: 24px; line-height: 1.6;">
      ...
      <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px; font-size: 13px;">
        <tr>
          <td style="padding: 7px 0; color: #64748b; font-weight: 600; width: 150px;">Sheet ID:</td>
          <td style="padding: 7px 0; color: #0f172a; font-family: monospace;">${transfer_recon_id}</td>
        </tr>
        ${vehicle_reg ? `
        <tr>
      ...
      <div style="background-color: #f0f9ff; border: 1px solid #bae6fd; border-radius: 8px; padding: 16px; font-size: 12px; color: #0c4a6e; text-align: center;">
        ...
        <p style="margin: 0;">Log in to the Admin Dashboard → Transfer Recon section to <strong>Approve</strong> or <strong>Reject</strong> this request.</p>
      </div>
    </div>
    ...
    const resendResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: fromEmail,
        to: toEmails,
        subject: `✏️ Transfer Recon Edit Request — ${driver_name ?? driver_id}`,
        text: plainText,
        html: htmlBody,
      }),
    });
    ...
    console.error('notify-transfer-edit-request error:', message);
