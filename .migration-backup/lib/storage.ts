...
export async function uploadToCloudinary(file: File, folder: string = 'inspections'): Promise<CloudinaryUploadResult> {
  ...
  try {
    ...
    // Cloudinary's 'authenticated' delivery type is NOT supported for raw resources.
    // Raw files (PDFs, Word docs) must use type 'upload'. Only image/video support 'authenticated'.
    const deliveryType = isRaw ? 'upload' : (sigData.type || 'upload');
...
export async function getSignedUrlForView(input: string | any): Promise<string> {
  ...
  try {
    ...
    // For images, the unsigned Cloudinary URL works fine — skip the signing round-trip.
    // For raw/document resources (PDFs, Word, etc.) Cloudinary requires authentication,
    // so we must always go through the edge function.
    if (resourceType === 'image' && fallbackUrl && !publicId) {
      return fallbackUrl;
    }
