...
export default function DriverDashboard({ driver, onLogout }: DriverDashboardProps) {
  ...
  // Capture sheet details before clearing state
  ...
  try {
    ...
    // 2. Send admin alert email via notify-recon-edit-request
    const { error: alertError } = await supabase!.functions.invoke('notify-recon-edit-request', {
      body: {
        recon_id: reconId,
        driver_id: driver.driver_id,
    ...
    if (alertError) console.warn('Admin alert email failed:', alertError.message);

    // 3. Also send OTP email to admin for authorization
    const { error: otpError } = await supabase!.functions.invoke('send-otp-email', {
      body: {
        resource_type: 'recon_edit',
        resource_id: reconId,
        context_label: `Trip Recon Edit Request — ${reason}`,
      },
      headers: { Authorization: `Bearer ${session.access_token}` },
    });
